...
class GNNLayer(torch.nn.Module):
    def __init__(self, in_dim, out_dim, attn_dim, n_rel, n_ent, n_node_topk=-1, n_edge_topk=-1, tau=1.0,
        ...
        # Stage 2: Global Modulation Module
        self.global_modulate = nn.Sequential(
            nn.Linear(attn_dim * 2 + 1, attn_dim),  # Take the local results, hqr, and degree information as input.
            nn.ReLU(),
    ...
    def forward(self, q_sub, q_rel, hidden, edges, nodes, old_nodes_new_idx, batchsize):
        ...
        # Compute node degrees as structural features of the graph
        ...
        # Stage 2: Global Modulation (local features + h_qr + degree)
        global_input = torch.cat([local_feature, self.Wqr_attn(h_qr), edge_degrees],
        ...
        if self.n_edge_topk > 0:
            ...
            topk_index = torch.topk(edge_prob, self.n_edge_topk, dim=0).indices  # Use topk instead of argsort
        ...
        # extract sampled nodes and their embeddings
