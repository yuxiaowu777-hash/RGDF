...
class DataLoader:
    ...
    def get_neighbors(self, nodes, batchsize, mode='train'):
        ...
        # node_1hot: [self.n_ent, N_ent_of_all_batch_last]
        ...
        node_1hot = csr_matrix((np.ones(len(nodes)), (nodes[:, 1].cpu().numpy(), nodes[:, 0].cpu().numpy())), shape=(self.n_ent, nodes.shape[0]))  # 在 GPU 上创建稀疏张量
        ...
        ], dim=1).to(torch.long)  # [N_edge_of_all_batch, 4]
