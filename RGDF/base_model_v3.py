...
from models_v3 import GNNModel
from utils import *
...
class BaseModel(object):
    def __init__(self, args, loader):
        ...
        # Optional optimization: add support for mixed-precision training.
    ...
    def saveModelToFiles(self, best_metric, deleteLastFile=True):
        savePath = f'{self.loader.task_dir}/saveModel_v2/{self.modelName}-{best_metric}.pt'
    ...
    def train_batch(self, ):
        ...
        for i in tqdm(range(n_batch)):
            ...
            batch_indices = torch.arange(len(scores), device=device)
            target_indices = torch.tensor(triple[:, 2], dtype=torch.long, device=device)
            pos_scores = scores[batch_indices, target_indices]
